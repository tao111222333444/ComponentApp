...
public class Utils {
    ...
    public static long getLocalVersionCode(Context context){
        ...
        try {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                    //在Android 9.0 （P）及以上的版本中  versionCode 是用long类型来保存了  所以用getLongVersionCode这个方法获取
    ...
    public static String getChannelName(Context ctx) {
        ...
        return getChannelName(ctx, CHANNEL_KEY);
    }
    ...
    public static String getChannelName(Context ctx,String channelKey) {
        if (ctx == null || TextUtils.isEmpty(channelKey)) {
            return null;
        }
        //可以根据自己的配置修改  这个是友盟的
